...
def map_coordinates_to_values(points, axis_info, viewbox):
    ...
    if viewbox:
        ...
        # This is the line from your log: 224.42
...
def create_memory_array(y_coords, axis_info):
    ...
    # ===================================================================
    # THIS IS THE FIX FOR THE INVERSION (v13)
    # ===================================================================
    # The `y_coords` we receive are already inverted by map_coordinates_to_values
    # We just need to map them from their pixel range to the data range [y_min, y_max]
    
    y_coord_min_pixel = y_coords.min()
    y_coord_max_pixel = y_coords.max()
    y_coord_range = y_coord_max_pixel - y_coord_min_pixel

    if y_coord_range == 0:
        print("⚠ All Y coordinates are identical.")
        return np.full(len(y_coords), (y_min + y_max) / 2)

    y_normalized = (y_coords - y_coord_min_pixel) / y_coord_range
...
def plot_memory_graphs(html_file):
    ...
    # Pass y_coords (inverted pixel values) to memory_array
