...
def extract_axis_info(soup):
    ...
    FIXED: Uses orientation attribute from recharts-cartesian-axis-tick
    """
    axis_info = {
        'x_labels': [],
        'y_labels': [],
    ...
    # Find all axis tick groups - the exact structure from recharts
    ...
    for tick_group in tick_groups:
        ...
        # Get orientation to determine axis
        orientation = text_elem.get('orientation', '')
        
        # Get the label from tspan (or fallback to text)
...
def parse_time_label(time_str):
    ...
    FIXED: Now handles both "7 PM" and "6:30 PM" formats
    """
...
def create_memory_array(y_coords, axis_info):
    ...
    FIXED: Now handles "286.16MB" format with MB suffix
    """
    ...
    # Parse numeric labels - strip "MB", "GB", etc. suffixes
    y_values = []
    for label in axis_info['y_labels']:
        try:
            # Remove MB, GB, KB, B suffixes and parse
