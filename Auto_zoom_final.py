...
def map_coordinates_to_values(points, axis_info, viewbox):
    ...
    # DO NOT INVERT HERE. The inversion must be done in create_memory_array
    # using the axis label positions, not the path data.
...
def create_time_array(x_coords, axis_info):
    ...
    for x_pos, label in zip(axis_info['x_positions'], axis_info['x_labels']):
        ...
        # --- NEW LOGIC ---
        if int_pos not in unique_labels_map:
            unique_labels_map[int_pos] = label # Keeps the *FIRST* label found at this spot
            
    # Convert map to a list of (pos, label) tuples and sort by position
    paired_list = sorted(unique_labels_map.items())
    # This log was wrong in the last screenshot (it said 66)
    print(f"✓ Found {len(paired_list)} unique, sorted time labels (using 'keep-first' int() grouping)")
...
def create_memory_array(y_coords, axis_info):
    ...
    This version uses int() to de-duplicate labels and maps
    pixels based on LABEL POSITIONS.
    """
    ...
    for y_pos, label in zip(axis_info['y_positions'], axis_info['y_labels']):
        ...
        # --- NEW LOGIC ---
        if int_pos not in unique_y_labels:
            unique_y_labels[int_pos] = label # Keeps the *FIRST* label found at this spot
            unique_y_pos[int_pos] = y_pos   # Keeps the *FIRST* pixel position
    
    sorted_y_labels = [label for pos, label in sorted(unique_y_labels.items())]
    sorted_y_pos = [pos for pos, label in sorted(unique_y_pos.items())]
    
    print(f"✓ Found {len(sorted_y_labels)} unique y-axis labels (using 'keep-first' int() grouping)")
    ...
    if not sorted_y_labels or len(sorted_y_labels) < 2:
        ...
        # Fallback to simple normalization (likely incorrect, but better than nothing)
        y_normalized = (y_coords.max() - y_coords) / (y_coords.max() - y_coords.min())
    ...
    # --- NEW SCALING LOGIC (THE FIX) ---
    
    # Data values
    y_data_min, y_data_max = min(y_values), max(y_values)
    print(f"Memory range: {y_data_min} to {y_data_max} MB")
    
    # Pixel positions of the axis labels
    # SVG y=0 is at the top
    y_pixel_bottom = max(sorted_y_pos) # e.g., 224.4 (for "0B")
    y_pixel_top = min(sorted_y_pos)    # e.g., 21.4 (for "572.20MB")
    y_pixel_range = y_pixel_bottom - y_pixel_top # e.g., 224.4 - 21.4 = 203
    
    print(f"Pixel range of axis: {y_pixel_top} (for {y_data_max}MB) to {y_pixel_bottom} (for {y_data_min}MB)")

    if y_pixel_range == 0:
        print("⚠ All Y coordinates are identical.")
        return np.full(len(y_coords), (y_data_min + y_data_max) / 2)

    # Invert and map in one step
    # (y_pixel_bottom - y) gives the pixel distance from the bottom axis
    # Divide by y_pixel_range to get a normalized value [0, 1]
    y_normalized = (y_pixel_bottom - y_coords) / y_pixel_range
    
    # Map normalized value to the data range
    memory = y_data_min + y_normalized * (y_data_max - y_data_min)
...
def plot_memory_graphs(html_file):
    ...
    unique_y_labels = {}
    for y_pos, label in zip(axis_info['y_positions'], axis_info['y_labels']):
        if int(y_pos) not in unique_y_labels:
    ...
    # --- THIS IS THE FIX ---
    # The SVG path contains baseline points. We must find the "real" end of the data.
    # We find the index of the point with the maximum x-coordinate.
    real_end_index = np.argmax(x_coords)
    
    time_axis_min = time_array[0] # This corresponds to x_coords[0]
    time_axis_max = time_array[real_end_index] # This corresponds to x_coords.max()
    total_seconds = (time_axis_max - time_axis_min).total_seconds()
    print(f"✓ Calculated real duration: {total_seconds} seconds")
    # --- END FIX ---
    ...
    # Figure 2: Zoomed view (6:30 PM - 7:00 PM)
    fig2, ax2 = plt.subplots(figsize=(16, 6))
    
    # Determine the correct date for zoom
    # The zoom window is at the END of the 24-hour graph
    base_date = time_axis_max.replace(hour=0, minute=0, second=0, microsecond=0)
    zoom_start = base_date.replace(hour=18, minute=30) # 6:30 PM
    zoom_end = base_date.replace(hour=19, minute=0)    # 7:00 PM

    # Handle edge case where graph ends *before* this zoom window
    # (e.g. 8 AM to 8 AM graph)
    if time_axis_max < zoom_start:
    ...
    if mask.any() and total_seconds > 0:
        ...
        ax2.set_title('Memory Usage - Magnified View (6:30 PM - 7:00 PM)', fontsize=15, fontweight='bold', pad=20)
        ...
        # ===================================================================
        # THIS IS THE FIX (v15) - REMOVED THE BAD Y-AXIS LIMIT
        # ===================================================================
        # ax2.set_ylim(y_axis_min, y_axis_max) # <--- REMOVED THIS LINE
        # ===================================================================
        
        print(f"✓ Zoomed view: {len(zoom_times)} points between 6:30-7:00 PM")
    else:
    ...
    print("\n" + "="*70)
    print("✅ COMPLETE!")
    print("="*7m    print(f"📊 Total data points: {len(points)}")
