...
# ===================================================================
# THIS IS THE CORRECTED FUNCTION - NOW USES X_POSITIONS!
# ===================================================================
def create_time_array(x_coords, axis_info):
    """
    Create time array from x coordinates and axis labels
    NOW USES x_positions for accurate mapping (like Y-axis)
    """
    ...
    for label in axis_info['x_labels']:
        t = parse_time_label(label) # Uses robust parser
        if t:
    ...
    # --- Detect midnight crossings (e.g., 11 PM -> 1 AM) ---
    adjusted_times = []
    ...
    times_parsed = adjusted_times
    # --- END midnight crossing detection ---
    ...
    # === NEW: Use x_positions for accurate mapping (same as Y-axis) ===
    if axis_info['x_positions'] and len(axis_info['x_positions']) == len(times_parsed):
        print("✓ Using x_positions for accurate time mapping")
        
        # Convert times to seconds from start_time
        x_positions = np.array(axis_info['x_positions'])
        time_seconds = np.array([(t - start_time).total_seconds() for t in times_parsed])
        
        # Interpolate using actual label positions (same approach as Y-axis)
        time_interp = np.interp(x_coords, x_positions, time_seconds)
        times = np.array([start_time + timedelta(seconds=s) for s in time_interp])
        
        return times
    # === END NEW ===
    
    # Fallback: Linear interpolation if x_positions not available
    print("⚠ x_positions not available, using linear interpolation")
    total_duration = (end_time - start_time).total_seconds()
    
    # Handle case where start/end are the same
...
def plot_memory_graphs(html_file):
    ...
    time_array = create_time_array(x_coords, axis_info) # NOW USES x_positions!
    ...
    # Get min/max values from parsed labels for setting axes
    y_labels_numeric = []
    for label in axis_info['y_labels']:
        try:
    ...
    # Set Y-axis limits from labels, with 0 as floor
    ...
    if time_array[0].day != time_array[-1].day:
        if time_array[0].hour > 12 and time_array[-1].hour < 12:
             if zoom_start.hour < 12:
