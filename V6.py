...
def create_memory_array(y_coords, axis_info):
    ...
    # Linear mapping
    # Your log showed: Y range: 88.78 to 224.42
    # We must use the de-duplicated y-positions to find the "real" pixel range
    unique_y_pos = {}
    for y_pos in axis_info['y_positions']:
        if int(y_pos) not in unique_y_pos:
            unique_y_pos[int(y_pos)] = y_pos
            
    visible_y_pos = list(unique_y_pos.values())
    y_coord_min = min(visible_y_pos)
    y_coord_max = max(visible_y_pos)
    
    # Now invert based on *these* values
    # e.g., y_coord_max = 224.42 (from "0B")
    # e.g., y_coord_min = 21.42 (from "572.20MB")
    
    # Invert Y coordinates (SVG is top-down)
    y_coords_inverted = y_coord_max - y_coords
    
    y_coord_range = y_coord_max - y_coord_min # e.g., 224.42 - 21.42 = 203
    
    if y_coord_range == 0:
        print("⚠ All Y coordinates are identical.")
        return np.full(len(y_coords), (y_min + y_max) / 2)

    # Map from inverted pixel range [0, 203] to data range [0, 572.20]
    y_normalized = (y_coords_inverted - (y_coord_max - y_coord_max)) / y_coord_range
    memory = y_min + y_normalized * (y_max - y_min)
    
    return memory

# ===================================================================
# THIS FUNCTION IS THE FIX (v12)
# ===================================================================
def plot_memory_graphs(html_file):
    ...
    if time_array[0].day != time_axis_max.day: # Use real end time
        if time_array[0].hour > 12 and time_axis_max.hour < 12: # e.g. starts at 8 PM, ends at 2 AM
